...
The [`session-sample-files/`](session-sample-files/) directory contains example files generated for a (simulated) processed session, useful for reference:

- `session_simulated_transcript.json` - Conversation synthetically generated for the session (for simulated sessions only)
- `session_audio.mp3` - Combined audio derived from a synthetically generated session (simulation) or a case uploaded by the user
- `session_audio_transcript.json` - Speaker-diarized transcription generated by gpt-4o-transcribe-diarize
- `session_data.csv` - Session data in CSV format extracted from the database after transcription processing
- `sql-query.txt` - SQL query used to extract the data from the database
