...
import { useReportPatientView } from "@/hooks/useReportSummaries";
import { AlertCircle, ArrowLeft, Calendar, HeartPulse } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useNavigate, useParams } from "react-router-dom";

export function ReportSummaryPatientViewPage() {
  const { t } = useTranslation();
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const { data, isLoading, isError } = useReportPatientView(id!);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Button
          variant="secondary"
          size="sm"
          onClick={() => navigate(`/report-summaries/${id}`)}
        >
